...
| `ToolNext` | Executes a tool continuation with JSON and captured scope, returning a JSON result or worker error. |
...
| RPC | Request Fields | Response Fields |
|---|---|---|
| `EmitMark` | `activation_id`, `auth_token`, captured `scope`, `name`, optional `data`, and optional `metadata` | `HostAck.ok` or `HostAck.error` |
...
| `ToolNext` | `activation_id`, `auth_token`, `continuation_id`, JSON `value`, and captured `scope` | JSON `value` or `error` |
...
| `nemo.relay.ToolExecutionInterceptOutcome@1` | Application tool result and Relay-owned pending marks. |
| `nemo.relay.PluginDiagnostics@1` | Diagnostics returned from worker validation. |

Surface results use empty, JSON, guardrail, LLM request outcome, tool execution outcome,
or structured error variants. Stream chunks contain one JSON value or one terminal
error. `WorkerError` carries a stable code, human-readable message, and retryable flag;
transport errors remain distinct from plugin callback errors.
