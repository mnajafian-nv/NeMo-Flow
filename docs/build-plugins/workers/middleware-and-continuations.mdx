...
and returns only the first result. The second call is intentional demonstration code: it
can still consume provider capacity, incur cost, and cause provider-side effects even
though Relay does not expose its result to the application.
