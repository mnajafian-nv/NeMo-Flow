...
all 15 surfaces, accepts synchronous and asynchronous callback forms where the
...
Relay creates the managed environment from the package root named by the manifest. A
...
dependencies = ["nemo-relay-plugin>=0.8.0,<0.9.0"]
...
class ExamplePythonWorker(WorkerPlugin):
    ...
    def register(self, ctx, config):
        ...
        if settings["execution"]["enabled"]:
            self._register_execution(ctx, settings["execution"])
...
the installed module. The digest shown is the checked value for the current `worker.py`;
changing that file requires a new digest.
...
[integrity]
sha256 = "sha256:1f678f03847b7634ce8b253be7eb09a7324741e9d399295b6542382c7d898bda"
...
The manifest uses `relay = ">=0.5,<1.0"` because `grpc-v1` is the compatibility
contract. The package still pins its own authoring dependency to the 0.8 SDK line shown
above; that is independent of the host versions that can negotiate this protocol.
...
2. Enable the component and start Relay.
   ...
   handshake should advertise all 15 supported surfaces.

## Verify Behavior and Cleanup
