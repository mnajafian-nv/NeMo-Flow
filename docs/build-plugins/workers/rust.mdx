...
The `examples/rust-grpc-worker-plugin` project is the compiled counterpart to the Python
worker. It depends on `nemo-relay-worker` 0.8.0, implements `WorkerPlugin`, registers all
15 surfaces, and lets the SDK create the authenticated `grpc-v1` server from the
...
executable, and integrity describes the exact executable that will start.
...
The manifest expresses the `grpc-v1` protocol range that the worker can speak, so the
checked worker uses `relay = ">=0.5,<1.0"`. Its crate dependency remains 0.8.0 because
that is the SDK version used to build this example; the broader manifest range is not a
claim that every SDK feature existed in every compatible Relay release.

The artifact and entrypoint paths must identify the same executable. Relay verifies the
artifact digest before it starts the command, and the worker handshake must return the
...
2. Start Relay from the repository root. Inspect the activation report
   ...
   `grpc-v1`, multiple-component behavior, and all 15 surfaces.
