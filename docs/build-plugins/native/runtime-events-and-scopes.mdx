...
The native runtime handle lets plugin middleware participate in Relay's event hierarchy
without linking the host runtime crate. The example's `runtime` group emits a configured
mark, opens and closes a child scope, and optionally creates an isolated scope stack.
