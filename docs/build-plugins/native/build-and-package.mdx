...
The repository example builds a `cdylib` against the current 0.8.0 SDK and packages it
...
dynamic boundary. The dependencies use the current 0.8.0 public SDK rather than a Git
...
plugin depends on the published `nemo-relay-plugin = "0.8.0"` package without `path`.
...
The checked manifest template declares the plugin identity, version contracts, schema,
