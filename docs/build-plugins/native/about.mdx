...
typed DTOs, registers all 15 plugin surfaces, and drives async middleware on one
...
impl NativePlugin for ExampleNativePlugin {
    ...
    fn register(
        ...
        requests::register(context, &config, &runtime)?;
...
Success is not merely a library that loads. The complete path validates rejected and
disabled input, activates a valid component, exercises the plugin's 15 registrations,
and verifies representative tool, LLM, stream, event, codec, continuation, mark, and
scope paths before it clears registrations and unloads the library.
