...
legacy v2 table. ABI v4 extends the complete v3 prefix with completion-scoped codecs and
pull-based downstream LLM streams. This table version is independent of authored
`compat.native_api = "1"`.
...
| Current v4 extension | Completion-scoped LLM request decode and encode plus response decode; pull-based downstream LLM stream open, pull, cancel, and release; completion retain for typed codec facades; and output-stream backpressure inspection. |
...
A downstream terminal callback reports failure or consumer cancellation through a
non-null error and clean completion with `done = true`. Reclaim its `user_data` in that
