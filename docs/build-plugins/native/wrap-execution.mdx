...
Execution intercepts receive the real request and a continuation representing the rest
...
The tool wrapper calls `next` once, awaits the downstream result, and returns a
`ToolExecutionInterceptOutcome` with the example's pending mark. Relay owns that mark:
it is emitted in the managed lifecycle and does not appear inside the JSON tool result.
...
If downstream returns `{"answer": 42}`, the application still receives
`{"answer": 42}`. Relay separately emits `example.native.tool_execution` under the
managed tool call. That separation is why the callback returns an outcome instead of
...
idempotent. The request flag makes repeated execution observable and opt-in for this
example.
