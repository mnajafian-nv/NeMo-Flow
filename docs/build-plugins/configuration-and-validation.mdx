...
| `relay-plugin.toml` | Package manifest for one discoverable native library or `grpc-v1` worker. It declares identity, compatibility, entrypoint, optional JSON Schema, and integrity metadata. It does not replace the component configuration in `plugins.toml`. |
...
The following complete schema is shared by the Python and Rust worker examples. The
native example adds an `executor` object because only the typed native SDK owns a Tokio
executor. `additionalProperties: false` is repeated inside each object so a misspelled
control fails package validation at the exact nesting level where it appears.
