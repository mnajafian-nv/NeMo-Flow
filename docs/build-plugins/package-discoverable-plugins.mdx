...
not the C host-table ABI number. The current SDK requests ABI v4 and the host retains
frozen v3 and v2 compatibility for older compiled plugins. For workers, declare
`compat.worker_protocol = "grpc-v1"`; the handshake still negotiates the exact protocol,
surfaces, authentication token, and lifecycle at startup.
...
2. Validate the component schema against valid, invalid, and disabled example
   configurations. Keep unknown-field behavior aligned between the schema and the
