...
```bash
curl -fsSL https://raw.githubusercontent.com/NVIDIA/NeMo-Relay/main/install.sh | NEMO_RELAY_VERSION=0.9.0 sh
```
...
```bash
cargo add nemo-relay@0.9.0
cargo add nemo-relay-adaptive@0.9.0
```
...
```bash
openclaw plugins install npm:nemo-relay-openclaw@0.9.0
...
```bash
uv add "nemo-relay[langchain,langgraph,deepagents]==0.9.0"
```
