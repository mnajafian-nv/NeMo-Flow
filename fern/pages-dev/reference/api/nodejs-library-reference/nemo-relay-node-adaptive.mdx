...
Merges caller-supplied overrides onto the opt-in LLM response and tool-result cache config shape (exact-match) used by the adaptive plugin. This is a section of the adaptive component, not a standalone plugin kind.
