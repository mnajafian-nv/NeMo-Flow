...
Merges caller-supplied overrides onto the opt-in LLM response and tool-result cache config shape used by the adaptive plugin. This is a section of the adaptive component, not a standalone plugin kind.

**Parameters**

- `config`: Partial response-cache settings to override.

**Returns**

A normalized response-cache config object.

**Remarks**

The default backend is in-memory; pass a `backend` (e.g. `redisBackend(url)`) for a shared cache. `bypassRate` defaults to `0.0`, while caching nondeterministic requests is opt-in.
...
### `ResponseCacheKeyStrategy` type alias
...
### `ResponseCacheKeyStrategy` constant
