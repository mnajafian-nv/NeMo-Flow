...
Converts `args` to JSON, runs the native tool execution lifecycle, and decodes only `result` back into the caller's typed result shape while preserving `annotation` unchanged.
...
- `options`: Optional scope handle, attributes, data, metadata, and toolCallId.

**Returns**

A promise resolving to the decoded typed result and opaque annotation.
